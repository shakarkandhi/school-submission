...
class Function(ScreenManager):
    def generate_qr_code(self,root):
        if self.ids.link_text.text != '':
            name=datetime.now()
            name=str(name.hour)+str(name.minute)+str(name.second)
            ...
            plyer.notification.notify(
                title = 'QR Code',message="Input Field is Empty"
            )
    ...
    def read(self,root):
        try:
            ...
            camera.export_to_png(f"{name}.png")
            data=decode(Image.open(f"{name}.png"))
            decMessage = fernet2.decrypt(data[0].data)
            decMessage = fernet1.decrypt(decMessage).decode()
            self.ids.output.text=decMessage
            os.remove(f"{name}.png")
            root.current="opt"
        except:
            plyer.notification.notify(
                title = 'QR Code',message="Try Again"
            )

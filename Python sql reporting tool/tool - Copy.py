...
def cmd():
    ...
    s1=r"<table border='1' class='dataframe'> \n <thead>\n  <tr style='text-align: right;'>\n <td><img src='C:\Users\IAGAUTOMATION\Desktop\Untitled.jpg' width=100px height=100px></td>\n<th> Batch</th>\n<td>"+str(batch)+"</td>\n</tr>\n<tr style='text-align: right;'>\n<th>Start</th>\n<td> "+str(td1)+"</td>\n<th>End</th><td>"+str(td2)+"</td>\n</tr>\n</table>"
    ...
    with open(fdt+'.html','w') as html_file:
        ...
        html_file.close()
    df.to_excel(fdt+'.xlsx',startrow=10)
    wb = openpyxl.load_workbook(filename=fdt+'.xlsx')
    ws = wb.get_sheet_by_name('Sheet1')
    ws.cell(row=1,column=1,value='Batch')
    ws.cell(row=1,column=2,value=batch)
    ws.cell(row=2,column=1,value='Start')
    ws.cell(row=2,column=2,value=td1)
    ws.cell(row=2,column=3,value='End')
    ws.cell(row=2,column=4,value=td2)
    wb.save(fdt+'.xlsx')
